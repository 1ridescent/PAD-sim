...
{
	// constants
	static int MONSTERS_STARTX = 0, MONSTERS_STARTY = 0, MONSTER_SIZE = 75;
	static int ORB_STARTX = 0, ORB_STARTY = MONSTERS_STARTY + MONSTER_SIZE, ORB_SIZE = 100;
	static int TEAM_STARTX = 0, TEAM_STARTY = ORB_STARTY + 5 * ORB_SIZE, TEAMMATE_SIZE = 100;
	static int NEWLINE_SIZE = 20;
	static int TEAM_HP_STARTX = 0, TEAM_HP_STARTY = TEAM_STARTY + TEAMMATE_SIZE;
	
	static int WINDOW_WIDTH = 6 * ORB_SIZE, WINDOW_HEIGHT = TEAM_HP_STARTY + NEWLINE_SIZE;
	...
	public void paintComponent(Graphics G)
	{
		...
		for(int i = 0; i < cur_state.floor.monsters.size(); i++)
		{
			...
			// set target area to be attribute color
			G.setColor(color_table[m.attr]);
			G.fillRect(MONSTERS_STARTX + i * MONSTER_SIZE, MONSTERS_STARTY, MONSTER_SIZE, MONSTER_SIZE);
			G.setColor(Color.black);
		...
		for(int i = 0; i < cur_state.team.members.size(); i++)
		{
			...
			// set target area to be attribute color
			G.setColor(color_table[t.attr.charAt(0)]);
			G.fillRect(TEAM_STARTX + i * TEAMMATE_SIZE, TEAM_STARTY, TEAMMATE_SIZE, TEAMMATE_SIZE);
			if(t.attr.length() > 1) // sub-attr
			{
				G.setColor(color_table[t.attr.charAt(1)]);
				G.fillRect(TEAM_STARTX + i * TEAMMATE_SIZE + TEAMMATE_SIZE / 2, TEAM_STARTY, TEAMMATE_SIZE / 2, TEAMMATE_SIZE);
			}
			G.setColor(Color.black);
			...
			G.drawString(damage_string, TEAM_STARTX + i * TEAMMATE_SIZE, TEAM_STARTY + 2 * NEWLINE_SIZE);
			
			G.drawString(t.skill.name, TEAM_STARTX + i * TEAMMATE_SIZE, TEAM_STARTY + 3 * NEWLINE_SIZE);
			G.drawString("cd: " + Integer.toString(t.skill.cur_cd), TEAM_STARTX + i * TEAMMATE_SIZE, TEAM_STARTY + 4 * NEWLINE_SIZE);
		}
		
		G.drawString("Team hp: " + cur_state.team.cur_hp + "/" + cur_state.team.max_hp + " (recover " + cur_state.team.get_final_recover(cur_state.board) + ")", TEAM_HP_STARTX, TEAM_HP_STARTY + NEWLINE_SIZE);
	}
	...
	public static void main(String[] args)
	{
		...
		color_table['B'] = new Color(0, 160, 160);
		color_table['G'] = new Color(0, 160, 0);
		color_table['L'] = new Color(160, 160, 0);
		color_table['D'] = new Color(160, 0, 160);
	    ...
	    game.setTitle("Dungeon Simulator");
