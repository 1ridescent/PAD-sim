...
public class game extends JPanel
{
	...
	public game()
	{
		...
		addMouseListener(new MouseAdapter()
		{
			...
			{
				if(busy) return;
				if(started) return; // can't really happen, but just to be safe...
				//System.out.println(e.getX() + " " + e.getY());
				
				if(0 <= e.getX() - TEAM_STARTX && e.getX() - TEAM_STARTX < cur_state.team.members.size() * TEAMMATE_SIZE
						&& 0 <= e.getY() - TEAM_STARTY && e.getY() - TEAM_STARTY < TEAMMATE_SIZE) // activate teammate skill
				{ // clicked a skill
					int i = (e.getX() - TEAM_STARTX) / TEAMMATE_SIZE;
					//System.out.println("activating " + i);
					Teammate t = cur_state.team.members.get(i);
					t.skill.activate(cur_state.floor);
	...
	public void paintComponent(Graphics G)
	{
		...
		for(int i = 0; i < cur_state.team.members.size(); i++)
		{
			...
			G.drawString("damage: " + t.get_final_damage(cur_state.team, cur_state.board, null), TEAM_STARTX + i * TEAMMATE_SIZE, TEAM_STARTY + 4 * NEWLINE_SIZE);
		}
