			...
			for(int c = 0; c < 6; c++)
			{
				if(!board[r][c].equals(".") && !visited[r][c])
				{
					...
					if(num_cleared > 0)
					{
						...
						damage[Character.getNumericValue(board[r][c].charAt(0))] += 0.25 * (num_cleared + 1);
						num_combos++;
					}
				}
			}
	...
	boolean clear_matches()
	{
		...
		for(int i = 0; i < 5; i++)
			for(int j = 0; j < 6; j++)
				if(match[i][j])
				{
					if(!cleared[Character.getNumericValue(board[i][j].charAt(0))])
					{
						cleared[Character.getNumericValue(board[i][j].charAt(0))] = true;
						num_colors++;
					}
					board[i][j] = ".";
					found_match = true;
